...
function setDefaults(config, json) {
    var name;
    var globalGitConfig = new GitConfig({global: true});
    var localGitConfig = new GitConfig();
    ...
    if (!json.homepage) {
      ...
      var homepage = localGitConfig.get('remote.origin.url');
      var pair;

      if (homepage) {

        pair = GitHubResolver.getOrgRepoPair(homepage);
    ...
    if (!json.authors) {
      ...
      var gitName = globalGitConfig.get('user.name');
      var gitEmail = globalGitConfig.get('user.email');
